...
static u32 crc32(BinaryReader& reader, u32 count) {
	u32 crc = 0xFFFFFFFF;
	for (u32 i = 0; i < count; i++) {
		crc = (crc >> 8) ^ crc32LUT[reader.read<u8>() ^ (crc & 0xFF)];
	}
	...
	while (true) {
		...
		switch (type) {
			...
			case IHDR:
				...
				bufImage = Image<Pixel::RGBA8>(width, height);
				...
				if (bitCountLUT[bitDepth] == 1) {
					...
					switch (colorType) {
						...
						default:
							throw ImageDecoderException("PNG Faulty color type");
			...
			case PLTE:
				...
				if (len % 3 || !len || len > 768) {
					throw ImageDecoderException("PNG Invalid PLTE chunk length");
				}

				for (u32 i = 0; i < len / 3; i++) {

					palette[i].setRed(reader.read<u8>());
